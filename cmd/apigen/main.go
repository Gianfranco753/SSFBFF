...
func generateConfigRoutes(routes []configRoute, pkg, genPkg string) ([]byte, error) {
	...
	for _, r := range routes {
		if r.ProxyURL != "" {
			...
			w("\t\tqueries := c.Queries()\n")
			w("\t\tif len(queries) > 0 {\n")
			w("\t\t\tvar queryBuilder strings.Builder\n")
			w("\t\t\tqueryBuilder.Grow(len(queries) * 16) // Pre-allocate capacity\n")
			w("\t\t\tfirst := true\n")
			w("\t\t\tfor k, v := range queries {\n")
			w("\t\t\t\tif !first {\n")
			w("\t\t\t\t\tqueryBuilder.WriteString(\"&\")\n")
			w("\t\t\t\t}\n")
			w("\t\t\t\tqueryBuilder.WriteString(k)\n")
			w("\t\t\t\tqueryBuilder.WriteString(\"=\")\n")
			w("\t\t\t\tqueryBuilder.WriteString(v)\n")
			w("\t\t\t\tfirst = false\n")
			w("\t\t\t}\n")
			w("\t\t\ttargetURL += \"?\" + queryBuilder.String()\n")
			w("\t\t}\n\n")
			...
			if r.RequestSchema != nil && r.RequestSchema.HasSchema() {
				...
				w("\t\t\treturn c.Status(400).JSON(fiber.Map{\"error\": err.Error()})\n")
				w("\t\t}\n\n")
			}
			...
			w("\t\t\t\treturn c.Status(resp.StatusCode).Redirect(location)\n")
			w("\t\t\t}\n")
			w("\t\t}\n\n")
